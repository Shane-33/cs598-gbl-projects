#!/usr/bin/env bash
# Exp3: Proteina stochastic sampling (sc), sc_scale_noise 0.60
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
PROTEINA_DIR="${PROJECT_ROOT}/hands_on_protein/third_party/proteina"
CONFIG_SRC="${PROJECT_ROOT}/hands_on_protein/configs/inference_ucond_200m_notri_sc_noise060.yaml"
CONFIG_DST="${PROTEINA_DIR}/configs/experiment_config/inference_ucond_200m_notri_sc_noise060.yaml"
...
cd "${PROTEINA_DIR}"
python proteinfoundation/inference.py --config_name inference_ucond_200m_notri_sc_noise060
