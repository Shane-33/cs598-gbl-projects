#!/usr/bin/env bash
# Exp2: Proteina deterministic / ODE sampling (vf)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
PROTEINA_DIR="${PROJECT_ROOT}/hands_on_protein/third_party/proteina"
CONFIG_SRC="${PROJECT_ROOT}/hands_on_protein/configs/inference_ucond_200m_notri_vf.yaml"
CONFIG_DST="${PROTEINA_DIR}/configs/experiment_config/inference_ucond_200m_notri_vf.yaml"
...
cd "${PROTEINA_DIR}"
python proteinfoundation/inference.py --config_name inference_ucond_200m_notri_vf
