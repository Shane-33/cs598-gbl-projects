...
HANDS_ON_ROOT="${PROJECT_ROOT}/hands_on_protein"
PROTEINA_DIR="${HANDS_ON_ROOT}/third_party/proteina"
...
echo "[info] Proteina dir:     ${PROTEINA_DIR}"
echo ""

if [[ ! -d "${PROTEINA_DIR}" ]]; then
  echo "[error] Proteina directory not found: ${PROTEINA_DIR}"
  echo "        Clone upstream Proteina into: ${PROTEINA_DIR}"
  exit 1
fi
...
if [[ ! -f "${PROTEINA_DIR}/environment.yaml" ]]; then
  echo "[error] Missing ${PROTEINA_DIR}/environment.yaml"
  exit 1
fi
...
echo "============================================================"
echo ""
echo "0) Tracked experiment YAMLs live in:"
echo "     ${HANDS_ON_ROOT}/configs/"
echo "   Run scripts copy them into Proteina before inference."
echo "   Ensure cloned Proteina still has inference_base.yaml with:"
echo "     ckpt_path: \${oc.env:CKPT_PATH}"
echo ""
...
echo "     CKPT_PATH"
echo ""
echo "3) At project root (${PROJECT_ROOT}), create .env from .env.example:"
