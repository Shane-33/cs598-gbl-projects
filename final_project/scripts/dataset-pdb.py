...
"""
mode = 'train_algorithm'
mlp_model = 'foldseek3di' # esm, protTrans
...
def load_model(mlp_model):
  ...
  elif mlp_model == 'foldseek3di':
    tokenizer = T5Tokenizer.from_pretrained('Rostlab/ProstT5', do_lower_case=False, use_safetensors=True)
...
def generate_3di_embeddings(df, model, tokenizer):
    results = {}
    df = df[df['3Di'].notna() & (df['3Di'] != 'N/A')].copy()

    # Build space-separated lowercase 3Di sequence per (pdb, chain_id)
    df['3di_seq'] = df.groupby(['pdb', 'chain_id'])['3Di'].transform(
        lambda x: ' '.join(x.str.lower())
    )

    proteins_with_seq = (
        df[['pdb', '3di_seq']].groupby('pdb')
        .apply(lambda x: list(np.unique(x['3di_seq']))[0])
        .to_dict()
    )
    seq_dict = sorted(proteins_with_seq.items(),
                      key=lambda kv: len(proteins_with_seq[kv[0]]), reverse=True)

    for seq_idx, (pdb_id, seq_spaced) in tqdm(enumerate(seq_dict, 1)):
        seq_len = len(seq_spaced.split())
        chain_id = df[(df['pdb'] == pdb_id) & (df['3di_seq'] == seq_spaced)].chain_id.unique()[0]

        # "<fold2AA>" prefix signals to ProstT5 that the input is a 3Di structural sequence
        prostt5_input = "<fold2AA> " + seq_spaced
        ...
        try:
            ...
            print(f"RuntimeError for {pdb_id} (L={seq_len})")
            continue

        # Position 0 is the <fold2AA> prefix token — strip it so rows align with residues
        emb = embedding_repr.last_hidden_state.detach().cpu().numpy().squeeze()[1:, :]
        assert emb.shape[0] == seq_len, \
            f"Embedding length {emb.shape[0]} != sequence length {seq_len} for {pdb_id}"

        results[pdb_id] = {chain_id: emb}
...
def create_3di_embeddings_dataset(df, embeddings, num_hidden_size):
    feat_cols = [f'Feature_{i+1}' for i in range(num_hidden_size)]
    df_clean = df[df['3Di'].notna() & (df['3Di'] != 'N/A')].copy()

    has_label = 'label' in df.columns
    meta_cols = ['pdb', 'chain_id', 'residue_letter', 'residue_number', '3di_token']
    all_columns = meta_cols + feat_cols + (['label'] if has_label else [])

    df_embeddings = pd.DataFrame(columns=all_columns)

    for protein in tqdm(embeddings.keys()):
        chain = list(embeddings[protein].keys())[0]
        prot_df = df_clean[(df_clean['pdb'] == protein) & (df_clean['chain_id'] == chain)]

        temp_df = pd.DataFrame(np.array(embeddings[protein][chain]), columns=feat_cols)
        temp_df.insert(0, 'pdb', protein)
        temp_df.insert(1, 'chain_id', prot_df['chain_id'].values)
        temp_df.insert(2, 'residue_letter', prot_df['residue_letter'].values)
        temp_df.insert(3, 'residue_number', prot_df['residue_number'].values)
        temp_df.insert(4, '3di_token', prot_df['3Di'].values)

        if has_label:
            temp_df['label'] = prot_df['label'].values.astype(bool)
...
def train_classifier(dataset, save_model=False):
 ...
 '1ES6', '3FSN', '1S6X', '1FFJ', '2DDR', '3RZN']
  ...
  meta_drop = [c for c in ['pdb', 'residue_1l', 'residue_letter',
                            'residue_number', 'chain_id', '3di_token', 'label']
               if c in dataset.columns]

  X_train, y_train = df_train.drop(meta_drop, axis=1).to_numpy(), df_train['label'].values
  X_test, y_test = df_test.drop(meta_drop, axis=1).to_numpy(), df_test['label'].values
  X_val, y_val = df_val.drop(meta_drop, axis=1).to_numpy(), df_val['label'].values
  X_extra_val, y_extra_val = df_extra.drop(meta_drop, axis=1).to_numpy(), df_extra['label'].values

  vc = df_train['label'].astype(int).value_counts()
  ratio = vc[0] / vc[1]
  ...
  if save_model:
    model_name = f'lgbm_model_pdb_{mlp_model}.pkl'
    save_dir = local_path + '/pLM-PMI-main/models/' if mlp_model == 'foldseek3di' else project_path + '/models/'
    joblib.dump(clf, open(save_dir + model_name, 'wb'))
...
if __name__ == "__main__":
  if mode == 'create_pdb_labeling':
      ...
      print("Starting generation of embeddings for model", mlp_model )
      print("--------------")
      if mlp_model == 'foldseek3di':
          df = load_dataset(local_path + '/PDB_dataset/pdb_dataset_all_filtered_3di.csv')
          model, tokenizer, num_hidden_size, num_hidden_layers, num_attention_heads = load_model(mlp_model)
          embeddings = generate_3di_embeddings(df, model, tokenizer)
          df = create_3di_embeddings_dataset(df, embeddings, num_hidden_size)
          save_dataset(df, local_path + '/PDB_dataset/pdb_dataset_all_foldseek3di.csv')
      else:
    ...
    if mlp_model == 'foldseek3di':
        df = load_dataset(local_path + '/PDB_dataset/pdb_dataset_all_foldseek3di.csv')

        # Prefix both to avoid column name collisions (3Di and AA alphabets are identical)
        df_aa = pd.get_dummies(df['residue_letter'], prefix='aa')
        df = df.merge(df_aa, left_index=True, right_index=True, how='inner')

        df_3di = pd.get_dummies(df['3di_token'], prefix='3di')
